...
- [ ] No Cloudflare production resources, Hetzner VM, or production
      application containers have been created yet.

## Phase 1: manual prerequisites

Complete these steps before running any provisioning command:

1. Open Cloudflare Zero Trust and activate the account if prompted.
...
```bash
set -a
source deploy/.env.infrastructure
set +a
...
- one Ubuntu 24.04 CX23 in `hel1`;
- an empty-inbound Hetzner firewall;
- one outbound Cloudflare Tunnel;
- DNS routes for `chat.johancarlin.com` and `ssh.johancarlin.com`;
- a Cloudflare Access SSH application with the interactive email one-time-PIN
  policy and GitHub Actions service-token policy;
- the deployment SSH key and recorded host key.
...
- Sign in at `https://chat.johancarlin.com` with Entra; verify the first user
...
- In the Agentgateway UI, verify each successful request contains model,
